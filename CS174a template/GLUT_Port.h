...
// IMPORTANT -- In the line below, add the filenames of any new images you want to include for textures!
		...
		const unsigned num_textures = 2;
		string texture_list[num_textures] = { "earth.tga", "text.tga" };
...
struct Debug_Screen : public DisplayObject
{
	KeyMap<Debug_Screen> keys;
	int start_index;
	map< string, string > debug_screen_strings;
	Text_Line* m_text;
	Debug_Screen() : start_index( 0 ), m_text(0) { }

	void next() { start_index++; if( start_index >= (int)debug_screen_strings.size() ) start_index = 0; }
	void prev() { start_index--; if( start_index < 0 ) start_index = debug_screen_strings.size() - 1; }
	void display( float animation_delta_time, Vector2d &window_steering )
	{
		glUniform4fv( g_addrs->color_loc, 			1, Vector4f( .9f,.9f,.9f,1 ).data() );
		if( !m_text ) m_text = new Text_Line ( 22,  Matrix4d::Identity()  );		// Define a max string size
		Matrix4d model_transform  = Affine3d( AngleAxisd( -PI/2, Vector3d( 0, 1, 0 ) ) ).matrix();
		model_transform *= Affine3d( Translation3d( 1, -.9, .9 )).matrix();
		model_transform *= Vector4d( 1, .075, .05, 1 ).asDiagonal();
		...
		for( int i = 0; i < 3; i++, it++ )
		{
			...
			m_text->draw_heads_up_display( Matrix4d::Identity(), Matrix4d::Identity(), model_transform );					// Comment this out to not display any strings on the UI
			model_transform *= Affine3d( Translation3d( 0, 1, 0 )).matrix();
		}
		for( auto s : all_controls )
		{
			m_text->set_string( s );
			m_text->draw_heads_up_display( Matrix4d::Identity(), Matrix4d::Identity(), model_transform );					// Comment this out to not list out any controls on the UI
			model_transform *= Affine3d( Translation3d( 0, 1, 0 )).matrix();
		}
		m_text->set_string( "Controls:" );
		m_text->draw_heads_up_display( Matrix4d::Identity(), Matrix4d::Identity(), model_transform );
	}
...
Graphics_Addresses::Graphics_Addresses( GLuint program ) 
{
	...
	diffuse_loc 		= glGetUniformLocation(program, "chalkiness");
	specular_loc 		= glGetUniformLocation(program, "shininess" );
	shininess_loc 		= glGetUniformLocation(program, "smoothness" );
...
public:
	vector< DisplayObject* > displayList;
	...
	int init( const char* vShader, const char* fShader )
	{
		...
		registerDisplayObject( debug_screen );
		
		return window_id;
	}

	void registerDisplayObject( DisplayObject& d )	{	displayList.push_back( &d );	d.init_keys();	 }
	void removeDisplayObject  ( DisplayObject& d )	{	displayList.erase( find( displayList.begin(), displayList.end(), &d ) );	}
