...
struct Tree : public Shape
{
	int basis_selection;
	Tree( const Matrix4d &points_transform, float time) : basis_selection(0)
	{
		populate( *this, points_transform, time);
		init_buffers();
	}
	...
	{
			Shape::draw(projection_transform, camera_transform, model_transform, texture_filename );
	}

	static void populate( Shape &recipient, const Matrix4d &points_transform, float time)
	{
		Matrix4d object_transform = Matrix4d::Identity();
		object_transform *= Vector4d( 1, -1, 1, 1 ).asDiagonal();
		drawOneAxis(recipient, object_transform, time);
		(new Sphere(Vector4d(5, 5, 5, 5).asDiagonal(), 3))->populate(recipient, object_transform, 0);
	}
private:
	static void drawOneAxis( Shape &recipient, Matrix4d &points_transform, float time)
	{
		Matrix4d original(points_transform), object_transform(points_transform);
		object_transform = original;
		int n = 20;
		for (int i = 0.5; i < 9.5; i++) {
			object_transform *= Affine3d(AngleAxisd(-PI/40 + (PI/400) * (abs(20 - ((int)time % 41))), Vector3d(0, 0, 1))).matrix();
			object_transform *= Affine3d(Translation3d(0, -1, 0)).matrix();
			Cube::populate(recipient, object_transform);
		}
		object_transform *= Affine3d(AngleAxisd(-PI / 40 + (PI / 400) * (abs(20 - ((int)time % 41))), Vector3d(0, 0, 1))).matrix();
		object_transform *= Affine3d(Translation3d(0, -1, 0)).matrix();
		points_transform = object_transform;
	}
};

struct Leg : public Shape
{
	int basis_selection;
	Leg(const Matrix4d &points_transform, float time) : basis_selection(0)
	{
		populate(*this, points_transform, time);
		init_buffers();
	}
...
private:
	static void drawOneAxis(Shape &recipient, Matrix4d &points_transform, float time)
	{
		...
		object_transform = original;
		int n = 0;
		object_transform *= Affine3d(Translation3d(0, 0, 0)).matrix();
		object_transform *= Affine3d(AngleAxisd(PI / 8 - (PI / 160) * (abs(20 - ((int)time % 41))), Vector3d(0, 0, 1))).matrix();
		object_transform *= Vector4d(0.5, 2, 0.5, 1).asDiagonal();
		Cube::populate(recipient, object_transform);
		object_transform = original;
		object_transform *= Affine3d(Translation3d(0, 1.8, 0)).matrix();
		object_transform *= Affine3d(AngleAxisd(PI / 8 - (PI / 160) * (abs(20 - ((int)time % 41))), Vector3d(0, 0, -1))).matrix();
		object_transform *= Vector4d(0.5, 2, 0.5, 1).asDiagonal();
		Cube::populate(recipient, object_transform);
	}
...
struct Wing : public Shape
{
	int basis_selection;
	Wing(const Matrix4d &points_transform, float time) : basis_selection(0)
	{
		populate(*this, points_transform, time);
		init_buffers();
	}
...
private:
	static void drawOneAxis(Shape &recipient, Matrix4d &points_transform, float time)
	{
		...
		object_transform = original;
		int n = 20;
		object_transform *= Affine3d(Translation3d(-1, 4, 0)).matrix();
		object_transform *= Affine3d(AngleAxisd(PI / 8 - (PI / 80) * (abs(20 - ((int)time % 41))), Vector3d(0, 0, 1))).matrix();
		object_transform *= Vector4d(5, 0.2, 5, 1).asDiagonal();
		Cube::populate(recipient, object_transform);
		object_transform = original;
		object_transform *= Affine3d(Translation3d(4, 4, 0)).matrix();
		object_transform *= Affine3d(AngleAxisd(PI / 8 - (PI / 80) * (abs(20 - ((int)time % 41))), Vector3d(0, 0, -1))).matrix();
		object_transform *= Vector4d(5, 0.2, 5, 1).asDiagonal();
		Cube::populate(recipient, object_transform);
	}
...
struct Bee : public Shape
{
	int basis_selection;
	Bee(const Matrix4d &points_transform, float animation_time) : basis_selection(0)
	{
		m_leg = new Leg(Matrix4d::Identity()*Affine3d(Translation3d(0.5, 2.2, 0)).matrix(), animation_time);
		m_leg1 = new Leg(Matrix4d::Identity()*Affine3d(Translation3d(0.5, 2.2, 2)).matrix(), animation_time);
		m_leg2 = new Leg(Matrix4d::Identity()*Affine3d(Translation3d(0.5, 2.2, -2)).matrix(), animation_time);
		m_leg3 = new Leg(-Matrix4d::Identity()*Affine3d(Translation3d(-2.5, -4, 2)).matrix(), animation_time);
		m_leg4 = new Leg(-Matrix4d::Identity()*Affine3d(Translation3d(-2.5, -4, 0)).matrix(), animation_time);
		m_leg5 = new Leg(-Matrix4d::Identity()*Affine3d(Translation3d(-2.5, -4, -2)).matrix(), animation_time);
		m_wing = new Wing(Matrix4d::Identity()*Affine3d(Translation3d(0, 2.6, 0)).matrix(), animation_time);
		m_sph = new Sphere(Matrix4d::Identity(), 3);
		m_sph2 = new Sphere(Matrix4d::Identity(), 3);
		m_cube = new Cube(Vector4d(2, 2, 7, 1).asDiagonal());
	}
	void timepassby(float animation_time) {
		*m_leg = Leg(Matrix4d::Identity()*Affine3d(Translation3d(0.5, 2.2, 0)).matrix(), animation_time);
		*m_leg1 = Leg(Matrix4d::Identity()*Affine3d(Translation3d(0.5, 2.2, 2)).matrix(), animation_time);
		*m_leg2 = Leg(Matrix4d::Identity()*Affine3d(Translation3d(0.5, 2.2, -2)).matrix(), animation_time);
		*m_leg3 = Leg(-Matrix4d::Identity()*Affine3d(Translation3d(-2.5, -4, 2)).matrix(), animation_time);
		*m_leg4 = Leg(-Matrix4d::Identity()*Affine3d(Translation3d(-2.5, -4, 0)).matrix(), animation_time);
		*m_leg5 = Leg(-Matrix4d::Identity()*Affine3d(Translation3d(-2.5, -4, -2)).matrix(), animation_time);
		*m_wing = Wing(Matrix4d::Identity()*Affine3d(Translation3d(0, 2.6, 0)).matrix(), animation_time);
	}
	// Only draw this set of axes if it is the one selected through the user interface.
	void draw(int current, const Matrix4d &projection_transform, const Matrix4d &camera_transform, const Matrix4d &model_transform, const std::string &texture_filename)
	{
		glUniform4fv(g_addrs->color_loc, 1, Vector4f(.9f, .6f, .2f, 1).data());			// Color
		m_leg->draw(current, projection_transform, camera_transform, model_transform, texture_filename);
		m_leg1->draw(current, projection_transform, camera_transform, model_transform, texture_filename);
		m_leg2->draw(current, projection_transform, camera_transform, model_transform, texture_filename);
		m_leg3->draw(current, projection_transform, camera_transform, model_transform, texture_filename);
		m_leg4->draw(current, projection_transform, camera_transform, model_transform, texture_filename);
		m_leg5->draw(current, projection_transform, camera_transform, model_transform, texture_filename);
		glUniform4fv(g_addrs->color_loc, 1, Vector4f(.9f, .0f, .0f, 1).data());			// Color
		m_wing->draw(current, projection_transform, camera_transform, model_transform, texture_filename);
		glUniform4fv(g_addrs->color_loc, 1, Vector4f(.2f, .6f, .4f, 1).data());			// Color
		m_sph->draw(projection_transform, camera_transform, model_transform*Affine3d(Translation3d(1.5, 5.7, -3.5)).matrix(), texture_filename, 3);
		m_sph2->draw(projection_transform, camera_transform, model_transform*Affine3d(Translation3d(1.5, 5.7, 3.5)).matrix(), texture_filename, 3);
		glUniform4fv(g_addrs->color_loc, 1, Vector4f(.5f, .2f, .4f, 1).data());			// Color
		m_cube->draw(projection_transform, camera_transform, model_transform*Affine3d(Translation3d(1.5, 5.7, 0)).matrix(), texture_filename);
	}
	Leg* m_leg;
	Leg* m_leg1;
	Leg* m_leg2;
